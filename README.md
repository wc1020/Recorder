...
加入之后可以改状态、打 0–10 分、写短评、填日期。电影、电视剧、图书共用同一套详情页。搜不到可以手动填标题加入。

| 类型 | 资料 | 状态文案 |
|------|------|----------|
| 电影 | TMDB（中文名、海报、年份、演职员） | 想看 / 看过 / 弃了 |
| 电视剧 | TMDB（同上，同一把 Key） | 想看 / 在看 / 看过 / 弃了 |
| 图书 | Google Books，对不上再 Open Library | 想读 / 在读 / 读过 / 弃了 |
| 游戏 | Steam | 想玩另记；玩过的主要看 Steam 时长和库存 |

### 电影、电视剧、图书

列表按子标签筛「全部」、对应状态和片单 / 书单。电影是想看 / 看过 / 弃了，电视剧多一个在看。卡片有年份、类型 / 作者等副标题。能按加入时间、评分、年份排序，能按资料里的类型筛。详情是资料卡（TMDB / Google Books / Open Library 快照，点刷新才更新），不是豆瓣分。搜不到可以手动添加。
...
- 只走 TMDB、Google Books、Open Library、Steam；不爬、不抓包、不走非官方代理
