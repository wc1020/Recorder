...
**当前进度：** 阶段 0、阶段 1 已完成。游戏页资料栏固定，内部分类：最近游玩 / 全部游玩 / 完美通关 / 库存 / 家庭库 / 想玩。Steam 私人/隐藏游戏全程排除。标签规则：最近游玩 = 近两周有时长且在账号库或家庭库，按近两周时长排；全部游玩 = 有时长且在账号库或家庭库，按总时长排；完美通关 = 全成就且在账号库或家庭库，按全成就完成时间排；库存 = 账号库；家庭库 = 家庭库且不在账号库。游戏卡片统一尺寸：封面、标题、总时长、近两周、成就、购入价/原价（未填购入价则按原价）。账号库存游戏的原价含已购 DLC；详情页列出全部 DLC，可单独填购入价，购入合计 = 本体（购入或原价）+ 已购 DLC（购入或原价），未购但填了购入价的也加进购入合计。全成就卡片有彩色边框。详情页资料栏：标题与封面顶对齐且只占一行（超出省略）；总时长 / 近两周小时数加粗；本体与合计两行始终显示；评价；简介最多三行（超出省略）；商店链接与封面底对齐。库存购入价在详情页填写。成就进度只用不过期的 Web API Key。私人/隐藏名单需要登录 token 才能向 Steam 拉新的；token 不可用时沿用上次备份，避免刷新后重新露出来。本会话第一次打开游戏页会拉 Steam 并写入 `local/steam-cache.json`；切 tab 只用这份备份，可用搜索框按名称筛当前列表；从详情返回时保留原来的 tab、筛选和滚动位置。刷新时拉取库存会带上 Steam 默认隐藏的未审核/成人向等游戏（`skip_unvetted_apps=false`）。点「刷新」才再请求。改 `local/.env` 里的 Steam token 后也要点「刷新」，进程会重新读文件。Steam 连不上时也用这份备份。换电脑时整夹拷走 `local/`（Key、数据库、Steam 备份），不进 git。下一步是阶段 2（标签、片单、排序、手动添加）。
...
| 路由 | 做什么 |
|------|--------|
| `/` | 电影 / 书：类型 + 状态筛选。游戏：资料栏 + 下列标签（隐藏游戏全程排除） |

游戏标签：

| 标签 | 规则 |
|------|------|
| 最近游玩 | 近两周有游玩时长，且在账号库或家庭库；按近两周时长排序 |
| 全部游玩 | 有游玩时长，且在账号库或家庭库；按总时长排序 |
| 完美通关 | 全成就，且在账号库或家庭库；按全成就完成时间排序 |
| 库存游戏 | 账号库 |
| 家庭库 | 家庭库中且不在账号库 |
