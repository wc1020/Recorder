...
**当前进度：** 阶段 0、阶段 1 已完成。Next.js + Prisma/SQLite 可启动；Item / Entry、三个 Provider、`/`、`/search`、`/item/[id]` 已通。下一步是阶段 2（标签、片单、排序、手动添加）。
...
| 路由 | 做什么 |
|------|--------|
| `/` | 按类型 Tab：电影 / 书 / 游戏；可按状态筛选；卡片：封面、标题、状态、星 |
| `/search` | 类型 + 关键词；结果可「加入」；已入库的标出来 |
| `/item/[id]` | 封面、简介、状态、评分、短评、日期；保存 Entry |
